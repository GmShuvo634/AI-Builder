...
- Copy `.env.example` to `.env.local` (or edit `.env`) and fill in secrets like `DATABASE_URL`, `CLERK_SECRET_KEY`, `INNGEST_API_KEY`, and `OPENAI_API_KEY` before running the app locally.
- `.env.local` is gitignored by default. Do not commit secrets.
...
- `E2B_API_KEY` and `E2B_TEMPLATE_ID` — required for sandbox creation in code generation flows
