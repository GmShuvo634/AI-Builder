...
export const devAgent = inngest.createFunction(
  {
    id: "code-agent",
    triggers: { event: "code-agent/run" },  // object, NOT array
  },
  async ({ event, step }) => {
    const sandboxId = await step.run("get-sandbox-id", async () => {
      const sandbox = await await Sandbox.create("u1yrkaokyjzef8qchho5");
